export const data = [
  {
    id: 1,
    message:
      "Make your life memorable. Thank you for making mine sweet and memorable.",
  },
  {
    id: 2,
    message:
      "Sending you a million smiles, one for each and every day, as I want you to keep smiling each and every day.",
  },
  {
    id: 3,
    message:
      "A friend is like a book that has to be read to appreciate its content. You are one of the finest people I have ever found. I wish for our friendship to continue forever.",
  },
  {
    id: 4,
    message:
      "You are the kind of friend that everyone needs to have, reliable and understanding.",
  },
  {
    id: 5,
    message:
      "Life is not about how many friends you have but the quality of the friends you have!",
  },
  {
    id: 6,
    message:
      "When I look at you, I see an understanding and reliable friend. This makes you a true friend.",
  },
  {
    id: 7,
    message:
      "Friendship is not how long you have been together, but how you value each other.",
  },
  {
    id: 8,
    message:
      "Thank you for touching my life in ways that you may never know. You are a precious gift sent from God.",
  },
  {
    id: 9,
    message:
      "If I was given one wish in life, I would give you the ability to see yourself through my eyes. You would realize how special you are to me.",
  },
  {
    id: 10,
    message:
      "In life people meet for various reasons. I am so glad to have met you in my life. We are best friends.",
  },
  {
    id: 11,
    message:
      "True friends are always for you today, tomorrow, and forever. They will always be there for you when you need them most.",
  },
  {
    id: 12,
    message:
      "The most important thing is life is quality and not quantity. In life, we strive to have reliable, good, and understanding friends. That’s why I have you as my friend.",
  },
  {
    id: 13,
    message:
      "You are more fun than anyone or anything I know, including bubble wrap.",
  },
  {
    id: 14,
    message: "You are the most perfect you there is.",
  },
  {
    id: 15,
    message: "You are enough.",
  },
  {
    id: 16,
    message: "You look great today.",
  },
  {
    id: 17,
    message: "You have the best smile.",
  },
  {
    id: 18,
    message: "Your outlook on life is amazing.",
  },
  {
    id: 19,
    message: "You just light up the room.",
  },
  {
    id: 20,
    message: "You make a bigger impact than you realize.",
  },
  {
    id: 21,
    message: "You have the best laugh.",
  },
  {
    id: 22,
    message: "I appreciate our friendship.",
  },
  {
    id: 23,
    message: "Your inside is even more beautiful than your outside.",
  },
  {
    id: 24,
    message: "You just glow.",
  },
  {
    id: 26,
    message: "You bring out the best in the rest of us.",
  },
  {
    id: 27,
    message: "You inspire me.",
  },
  {
    id: 28,
    message: "Nothing can stop you.",
  },
  {
    id: 29,
    message: "You just made my day.",
  },
  {
    id: 30,
    message: "I am a better person because of you.",
  },
];
